...
def refresh_board():
    ...
    for i in range(40):
        ...
        add_to_output(
            COLORS.backBLACK + color + f"\033[{board.locations[i].x};{board.locations[i].y}H{i}" + backcolor + " " * (
                        4 + (1 if i < 10 else 0))
            )
        ...
        if (board.locations[i].owner == -3):  # If community chest
            add_to_output(COLORS.RESET)
            add_to_output(
                f"\033[{board.locations[i].x + 1};{board.locations[i].y}H" + COLORS.COMMUNITY + "█" * 6
                )
            add_to_output(
                f"\033[{board.locations[i].x + 2};{board.locations[i].y}H" + COLORS.COMMUNITY + "▀" * 6
                )
        ...
        if (board.locations[i].houses > 0):  # If there are houses
            add_to_output(COLORS.RESET)
            add_to_output(
                f"\033[{board.locations[i].x + 2};{board.locations[i].y + 1}H" + COLORS.GREEN + "▀" * (
                    board.locations[i].houses)
                )
        ...
        if (board.locations[i].owner == -2):  # If mortgaged
            add_to_output(COLORS.RESET)
            add_to_output(
                f"\033[{board.locations[i].x + 2};{board.locations[i].y}H" + COLORS.backLIGHTGRAY + "M"
                )
...
def update_history(message: str):
    ...
    if "[38;5" in message:
        if (((40 - (len(message) - 9)) * 2) == 0):
            history.append(
                message[:9] + "─" * ((40 - (len(message) - 9)) // 2) + message[
                                                                       9:] + "─" * (
                            (40 - (len(message) - 9)) // 2)
                )
        else:
...
def update_status(p: MonopolyPlayer, update: str, status: list = status,
                  mode: str = "normal", property_id: str = ""):
    """
    Update the status\n
    """
    ...
    if (update == "deed"):
        ...
        try:
            ...
            if location.owner > -1:  # if the location is owned
                ...
                status.append(
                    f"Current owner: " + color + f"{players[location.owner]}" + COLORS.RESET
                    )
                status.append(f"Houses: {location.houses}")
            if (
                    location.rent != 0):  # if location could be owned and is not a utility or railroad
                ...
                status.append(f"Mortgage Value: {location.mortgage}")
                    ...
                    location.owner >= -2 and location.rent == 0):  # if is a railroad or utility
                status.append(f"{location.color}=== {location.name} ===")
                status.append(f"Purchase Price: {location.purchasePrice}")
                status.append(
            ...
            if mode == "banker":
                return get_gameboard() + ss.set_cursor_str(
                    0, 39
...
def refresh_h_and_s():
    ...
    for i in range(len(history)):
        add_to_output(
            f"\033[{i + 4};81H" + (
                history[i] if i < len(history) else "") + COLORS.RESET
            )

    # Refresh status
    for i in range(len(status)):
        add_to_output(
            f"\033[{i + 4};122H" + status[i] if i < len(status) else ""
            )
    add_to_output(COLORS.RESET)
    ...
    for i in range(len(sorted_players)):
        if (sorted_players[i].order != -1):
            ...
            add_to_output(
                color + f"\033[{31 + i};122H{sorted_players[i].name} - ${sorted_players[i].cash}" + COLORS.RESET
                )
...
def buy_logic(mode: str = "normal", pinput: str = ""):
    ...
    if mode == "normal":
        choice = input(
            ss.set_cursor_str(0, 37) + "b to buy, enter to continue?"
            )
    else:
        choice = pinput
    if (board.locations[CL].purchasePrice != 0 and board.locations[
        CL].owner == -1):
...
def housing_logic(p: MonopolyPlayer, mode: str = "normal", propertyid: str = "",
                  num_houses: int = -1):
    ...
    if mode == "normal":
        propertyid = input(
            ss.set_cursor_str(
                0, 39
                ) + "What property do you want to build on? Enter property # or 'e' to exit."
            )
    else:
        if propertyid == "e":
            return get_gameboard()
    ...
    try:
        ...
        add_to_output(
            f"\033[42;0" + COLORS.RED + f"Invalid input, please enter a number in {p.properties}" + COLORS.RESET
            )
        flag = False
    if flag and not exit_flag:
        if not propertyid in p.properties:
            ...
            if family == COLORS.CYAN or family == COLORS.LIGHTBLACK or \
                    board.locations[propertyid].name.startswith("Electric"):
                add_to_output("This property cannot be improved.")
                flag = False
                if mode == "banker":
                    return get_gameboard() + ss.set_cursor_str(
                        0, 40
                        ) + "This property cannot be improved."
            if flag:
                for i in range(
                        propertyid - 3 if propertyid > 3 else 0,
                        propertyid + 5 if propertyid < 35 else 39
                        ):  # check only a few properties around for efficiency
                    if board.locations[i].color == family:
                        if not i in p.properties:
                            add_to_output(
                                "You do not own a monopoly on these properties!"
                                )
                            flag = False
                            if mode == "banker":
                                return get_gameboard() + ss.set_cursor_str(
            ...
            if flag:
                ...
                if flag:
                    ...
                    if mode == "normal":
                        houses = input(
                            f"Cost of housing is ${cost}. How many houses would you like to buy? (Max {max_houses}/min 0)"
                            )
                    else:
                        return get_gameboard() + ss.set_cursor_str(
                            0, 39
                    ...
                    if mode == "normal":
                        try:
                            ...
                            add_to_output(
                                f"Invalid input. Please enter a number 0-{max_houses}"
                                )
...
def player_roll(num_rolls, act: int = 0, mode: str = "normal") -> str:
    ...
    if (players[turn].order != -1):  # If player is not bankrupt
        ...
        was_in_jail = players[
            turn].jail  # Flag to check if player was in jail before rolling

        if players[turn].jail:
            if players[turn].jail_turns < 3:
                while True:
                    choice = input(
                        f"\033[36;0HYou're in jail. Pay ${50 * players[turn].repeat_offender} fine (f) or attempt to roll doubles (r)" +
                        (
                            "\nYou have a Get Out of Jail Free card. (c) to use." if
                            players[turn].jail_cards > 0 else ".")
                        ).lower().strip()
                    if choice == 'f':
                        players[turn].pay_jail_fine()
                        update_history(
                            f"{players[turn].name} paid ${50 * players[turn].repeat_offender} to post bail."
                            )
                        break
                    elif choice == 'c' and players[turn].jail_cards > 0:
                        players[turn].use_jail_card()
                        update_history(
                            f"{players[turn].name} used a Get Out of Jail Free card."
                            )
                        break
                    elif choice == 'r':
                        update_history(
                            f"{players[turn].name} will attempt to roll doubles."
                            )
                        break
                    else:
                        update_history(
                            f"Invalid choice. Please enter (f) to pay fine or (r) to roll" +
                            (
                                ". You have a Get Out of Jail Free card. (c) to use." if
                                players[turn].jail_cards > 0 else ".")
                            )
                ...
                update_history(
                    f"This is {players[turn].name}'s third turn in jail. They must attempt to roll doubles or they will pay the fine."
                    )
        ...
        if players[turn].jail:
            ...
            if left_jail:
                if reason == "doubles":
                    update_history(
                        f"{players[turn].name} rolled doubles and got out of jail!"
                        )
                elif reason == "third_turn":
                    update_history(
                        f"{players[turn].name} didn't roll doubles on their third turn. They paid ${50 * players[turn].repeat_offender} to post bail."
                        )
                    players[turn].pay_jail_fine()
            else:
                update_history(
                    f"{players[turn].name} didn't roll doubles and is still in jail. Turns in jail: {players[turn].jail_turns}"
                    )
                return
        ...
        if dice[0] == dice[1] and not was_in_jail:
            if num_rolls == 1:
                ...
                update_history(
                    f"{players[turn].name} rolled doubles three times in a row!"
                    )
        ...
        if players[turn].jail == False:
            if (players[turn].location + dice[0] + dice[
                1]) > 39:  # checks if player passed go
            ...
            if board.locations[players[turn].location].owner < 0:
                match board.locations[players[turn].location].owner:
                    ...
                    case -3:  # community chest
                        ...
                        if old_loc > new_loc and new_loc != 10 and new_loc != \
                                players[
                                    turn].location - 3:  # check if chance card made player pass go
                            update_history(
                                f"{players[turn]} passed Go and received $200"
                                )
                    case -4:  # chance
                        old_loc = players[turn].location
                    ...
                    case -10:  # go
                        pass
            elif board.locations[players[turn].location].owner != players[
                turn].order:
                # Pay another player rent
                cl = players[turn].location
                rent = board.locations[cl].getRent()
                if board.locations[cl].name == "Electric Company" or \
                        board.locations[cl].name == "Water Works":
                    rent *= dice[0] + dice[1]
...
def process_roll(num_rolls: int, dice: tuple) -> str:
    ...
    if dice[0] == dice[1]:
        if num_rolls == 1:
            ...
            update_history(
                f"{players[turn]} rolled doubles three times\n in a row!"
                )
...
def evaluate_board_location(num_rolls: int, dice: tuple) -> str:
    ...
    if dice[0] == dice[1]:  # and not was_in_jail:
        num_rolls += 1
        request_roll()
    return "player_choice" + ss.set_cursor_str(
        0, 38
        ) + "e to end turn, p to manage properties, d to view a deed?" + get_gameboard()
...
def player_choice():
    ...
    if (players[turn].cash > 0):
        ...
        update_history(
            f"{players[turn]} is in debt. Resolve debts before ending turn."
        )

        while True:
            option = None
            while option not in ["b", "m", "s"]:
                option = input(
                    "\033[38;0HResolve debts before ending turn. \nEnter b to "
                    "bankrupt, m to mortgage a property, or s to sell "
                    "houses/hotels: "
                ).lower().strip()
            ...
            if option == "b":  # Declare bankruptcy
                ...
                property_id = input(
                    "\033[38;0HEnter property number to mortgage: "
                ).lower().strip()
                ...
                while True:
                    try:
                        ...
                        property_id = input(
                            "\033[38;0HError -- not an integer: Enter property "
                            "number to mortgage: "
                        ).lower().strip()
                ...
                if own_property(turn, property_id):
                    ...
                    update_history(
                        f"You do not own property number {property_id}; did NOT"
                        f" mortgaged."
                    )

            elif option == "s":  # Sell houses/hotels
                property_id = input(
                    "\033[38;0HEnter property number to property: "
                ).lower().strip()
                ...
                if own_property(turn, property_id):
                    if property_has_houses(property_id):
                        update_history(f"{players[turn]} sold houses.")
                        break
                    else:
                        update_history(f"You do not own have any houses on "
                                       f"property number {property_id}")
                else:
                    update_history(
                        f"You do not own property number {property_id}; "
                        f"did NOT sell houses."
                        )
...
def own_property(turn, property_id):
    return board.locations[property_id].owner == turn


def property_has_houses(property_id):
    return board.locations[property_id].houses > 0
